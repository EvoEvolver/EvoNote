...
def delete_old_comment_output(manager: SrcManager, caller_id, line_i: int,
                              evolver_id: str):
    # Check whether it's the last line
    # Or the next line isn't generated by the evolver
    if line_i >= manager.src_len - 1 or manager.src_list[
        line_i + 1].strip() != f'{comment_delimiter}{evolver_id}':
        return
    start = line_i + 1
    end = -1
